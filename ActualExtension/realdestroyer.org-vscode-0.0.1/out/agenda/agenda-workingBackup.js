                    ...
                    if (items[i].includes(".org")) {
                        ...
                        fileText.forEach(element => {
                            if (element.includes("SCHEDULED") && !element.includes("DONE")) {
                                ...
                                taskText = taskText[0].replace(/⊙|⊘|⊖|TODO|DONE|IN_PROGRESS|CONTINUED|ABANDONED/g, "").trim();
                                getDateFromTaskText = element.match(/\[(.*)\]/);
        
                                if (taskTextGetTodo !== null) {
                                    taskText =
                                        '<span class="filename">' + items[i] + ":</span> " +
                                        '<span class="todo" data-filename="' + items[i] + '" data-text="' + taskText + '" data-date="' + getDateFromTaskText[0] + '"> ' +
                        ...
                        // ** SORT THE OBJECT BY DATE PROPERLY **
