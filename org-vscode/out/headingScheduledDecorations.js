...
const HEADING_CLOSED_DECORATION_SCOPE = "meta.decoration.heading.closed.vso";
...
function shouldDecorateClosed(editor) {
  if (!editor) return false;
  if (!editor.document) return false;
  if (editor.document.languageId !== "vso") return false;

  const config = vscode.workspace.getConfiguration("Org-vscode");
  return Boolean(config.get("decorateHeadingClosedDates", false));
}
...
function computeClosedDecorationsForEditor(editor) {
  const decorations = [];
  if (!editor) return decorations;
  ...
  for (const visibleRange of visible) {
    ...
    for (let lineNumber = startLine; lineNumber <= endLine; lineNumber++) {
      ...
      const planning = getPlanningForHeadingInDocument(document, lineNumber);
      if (!planning || !planning.closed) continue;

      const closedMoment = momentFromTimestampContent(planning.closed, acceptedDateFormats, true);
      if (!closedMoment || !closedMoment.isValid()) continue;

      const closedFormatted = closedMoment.format(dateFormat);
      const insertAt = new vscode.Position(lineNumber, lineText.length);
      const range = new vscode.Range(insertAt, insertAt);
      decorations.push({
        range,
...
function registerHeadingClosedDecorations(ctx) {
  if (!vscode.window || typeof vscode.window.createTextEditorDecorationType !== "function") {
    return;
  }
  ...
  let closedDecorationType = null;
  let closedDecorationKey = "";

  ctx.subscriptions.push({
    dispose() {
      if (closedDecorationType) {
        try { closedDecorationType.dispose(); } catch (_) { /* ignore */ }
        closedDecorationType = null;
      }
    }
  });

  function getClosedDecorationType() {
    const style = styleResolver.getStyleForScope(HEADING_CLOSED_DECORATION_SCOPE);
    const key = styleKey(style);
    if (closedDecorationType && key === closedDecorationKey) return closedDecorationType;

    if (closedDecorationType) {
      try { closedDecorationType.dispose(); } catch (_) { /* ignore */ }
    }

    closedDecorationKey = key;
    closedDecorationType = vscode.window.createTextEditorDecorationType(decorationTypeOptionsFromStyle(style));
    return closedDecorationType;
  }
  ...
  function clearEditor(editor) {
    if (!editor) return;
    const type = getClosedDecorationType();
    editor.setDecorations(type, []);
  }

  function apply(editor) {
    if (!editor) return;
    if (!shouldDecorateClosed(editor)) {
      clearEditor(editor);
      return;
    }

    const type = getClosedDecorationType();
    const decorations = computeClosedDecorationsForEditor(editor);
    editor.setDecorations(type, decorations);
  }
  ...
  ctx.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (
        event.affectsConfiguration("Org-vscode.decorateHeadingClosedDates") ||
        event.affectsConfiguration("Org-vscode.dateFormat") ||
        event.affectsConfiguration("editor.tokenColorCustomizations")
      ) {
        if (event.affectsConfiguration("editor.tokenColorCustomizations")) {
...
module.exports = {
  ...
  registerHeadingClosedDecorations
};
