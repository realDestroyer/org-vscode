...
/**
 * Pure function: Transform a day heading line by adjusting its date.
 * Returns { text, parseError } where text is the new line (or null if no match),
 ...
 */
function transformDayHeadingDate(text, forward, dateFormat, acceptedDateFormats) {
    const match = text.match(DAY_HEADING_REGEX);
    if (!match) {
...
function incrementDate(forward = true) {
    ...
    for (const lineNumber of sortedLines) {
        ...
        const result = transformDayHeadingDate(line.text, forward, dateFormat, acceptedDateFormats);
