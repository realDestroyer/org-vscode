...
const { classifyContext, commandForContext } = require(
...
module.exports = {
  ...
  run: () => {
    ...
    assert.strictEqual(classifyContext('- [ ] Task', 5), 'list');
    assert.strictEqual(classifyContext('- [ ] Task', 7), 'list');
    ...
    assert.strictEqual(classifyContext('  1. Item', 5), 'list');
    ...
    assert.strictEqual(commandForContext('table'), 'org-vscode.insertNewElement');
