...
| Decorative Unicode Headings | Render Unicode markers via editor decorations while keeping `*` headings as the source of truth | In Progress | vNext | realDestroyer |
| Configurable Date Format  | Ensure all date parsing/writing respects `Org-vscode.dateFormat` across commands/views     | In Progress | vNext | realDestroyer |
