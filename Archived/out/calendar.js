...
exports.openCalendarView = openCalendarView;
...
function openCalendarView() {
    ...
    calendarPanel = vscode.window.createWebviewPanel("calendarView", "Calendar View", vscode.ViewColumn.Beside, { enableScripts: true });
...
function sendTasksToCalendar(panel) {
    let tasks = [];
    let dirPath = vscode.workspace.getConfiguration("Org-vscode").get("folderPath") || path.join(require("os").homedir(), "OrgFiles");
...
function rescheduleTask(file, oldDate, newDate, taskText) {
    let filePath = path.join(vscode.workspace.getConfiguration("Org-vscode").get("folderPath") || path.join(require("os").homedir(), "OrgFiles"), file);
